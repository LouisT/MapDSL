...
class MapDSL extends Map {
      ...
      /*
       * Allow MapDSL to generate an incremented key if key is omitted.
       */
       ...
       *  avoid this.constructor.<name> calls within the MapDSL library itself.
       */
      ...
      find (queries = {}, projections = {}, one = false) {
          if (Helpers.is(queries, '!object')) {
             if (this.has(queries)) {
                return new Cursor(queries).add(new Document(queries, this.get(queries)));
             }
             queries = { '$eq' : queries };
          }
          let _queries = this.compileQueries(queries);
          if (!!_queries.list.length) {
             let cursor = new Cursor(queries);
             for (let entry of this.entries()) {
                 if (this._validate(entry, _queries)) {
                    cursor.add(new Document(entry[0], entry[1]));
                    if (one) {
                       return cursor;
                    }
                 }
