...
class ChainManager {
      constructor (MapDSL) {
          ...
          Object.keys(this.MapDSL.constructor.queryOperators).forEach((qo) => {
          ...
          Object.keys(this.MapDSL.constructor.logicalOperators).forEach((lo) => {
      ...
      execute (projections = {}, one = false) {
          return this.MapDSL[(one ? 'findOne' : 'find')](this.query, projections);
      }

      /*
       * Generate a chainable object.
       */
      _getOperatorChain (key, val = null, $qo = '$eq') {
          try {
              return this.MapDSL.constructor.queryOperators[$qo].chain(key, val, $qo);
           } catch (error) {
              return (val ? { [key] : { [$qo]: val } } : { [$qo]: key });
          }
      }
